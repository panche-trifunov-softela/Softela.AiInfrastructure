 ...
 * all — runs `cd Softela.ReactSCExpert && npx tsc --noEmit`. Before the
 * fix, `ctx.cwd` stayed the parent, `resolveProject` matched nothing, fell
 * back to `_default`, and `typecheck-invocation` — which is entirely
 * project-configured — never even had a `commands.typecheck.deny` pattern to
 * test against, so the known-bad invocation passed silently.
 *
 ...
 * named `Softela.ReactSCExpert`, git-initialised with a remote matching
 * `projects/Softela.ReactSCExpert.json`'s own `match.remotes` pattern —
 ...
 * directory, and the frontend repository nested inside it.
 */
function buildScratchCheckout(tmpdir) {
  const parent = tmpdir();
  const repo = path.join(parent, "Softela.ReactSCExpert");
  fs.mkdirSync(repo);
  execFileSync("git", ["init", "-q"], { cwd: repo });
  execFileSync(
    "git",
    ["remote", "add", "origin", "https://dev.azure.com/org/Project/_git/Softela.ReactSCExpert"],
    { cwd: repo },
  );
  return { parent, repo };
}

/** The known-bad invocation `projects/Softela.ReactSCExpert.json`'s own `commands.typecheck.deny` pattern matches. */
const TRAP_COMMAND = "cd Softela.ReactSCExpert && npx tsc --noEmit";

suite("adapters/shell-workdir-field-test", ({ test, eq, ok, tmpdir }) => {
  test("Claude Code: cd Softela.ReactSCExpert && npx tsc --noEmit reaches typecheck-invocation from the PARENT directory", () => {
    ...
    ok(reason.includes("typecheck-invocation"), "reason should name the rule that actually fired");
  });

  test("Codex: an exec call wrapping the same trap reaches typecheck-invocation from the PARENT directory", () => {
    const { parent } = buildScratchCheckout(tmpdir);
    const home = tmpdir();
