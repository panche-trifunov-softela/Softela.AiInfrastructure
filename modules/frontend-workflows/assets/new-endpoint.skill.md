---
name: "new-endpoint"
description: "Add a frontend call to a backend endpoint the agreed way: the contract taken from the backend itself rather than from a similar-looking frontend file, the route as a constant, the method typed end to end, and the call reachable only through a hook. Use when asked to add, wire up or call a new API endpoint, service method or backend request from the frontend. Do not use it for backend work, and do not use it to guess at an endpoint or a field that cannot be found — stop and ask instead."
---
