...
/**
 * Resolves this module's `core/lib` dependency (`hook-stdin`) across its two
 * on-disk layouts, so `inject-plan-gate.js` never reaches into `core/lib`
 * directly.
 *
 * A hook script in this module lives at `modules/analyze-first/hooks/x.js`
 * in the repository, three directory levels above `core/lib/`, but is
 * copied flat to `<agentHome>/softela-ai/hooks/x.js` when installed
 * (`core/installer/detect.js`'s `mod.json.files` loop), one level above the
 * `core/` payload installed alongside it at `<agentHome>/softela-ai/core/`. Both
 * paths are tried in turn for each dependency; resolving neither is a hard
 * failure rather than a silent fallback, matching
 * `modules/memory-as-context/hooks/stdin.js`, the precedent this shim
 * follows.
 *
 ...
 *
 * @param {string} name The `core/lib` module's base name, e.g. `"hook-stdin"`.
...
function resolveCoreLib(name) {
  ...
  try {
    ...
    if (!err || err.code !== "MODULE_NOT_FOUND") throw err;
    throw new Error(
      `analyze-first-core-lib.js: could not resolve core/lib/${name} from either the ` +
