...
`Softela.Bugworx` is the React frontend for **Pest Management** — the same product the .NET backend `Softela.PestManagement` serves. The link is verifiable, not inferred: Keycloak realm `pestmanagement`, client `pestmanagement-webapp`, API base defaulting to the Aspire host's `localhost:5250`. The backend's own facts live in `docs/projects/Softela.PestManagement/README.md`.
...
- **Bootstrap 5**, SCSS under `src/assets/scss`. No MUI, no CSS modules, no component library — styling is Bootstrap utility classes plus the vendored theme.
...
[[softela-git-flow]] carries the portable rules; these are the facts it defers to this repository for:

- **Base branch is `master`.** Never assume the base branch another repository uses.
